"""
DHRUVA Scraped Data Normalization and Relational Database Loader
Extracts, cleans, normalizes scraped dataset into relational tables (SQLite + CSVs)
...
def clean_image_url(place_id: str, image_urls: list) -> str:
    ...
    # Try any Scene7 that is not placeholder
...
def process_and_populate():
    ...
    for idx, p in enumerate(filtered_places, start=1):
        ...
        # OPENING_HOURS table rows (all 7 days with standardized slots)
    ...
    # 6. Define Sample USERS_INPUT (Demonstrates Itinerary Input Schema)
    users_input_data = [
    ...
    # --- 7. Create and Populate SQLite Database ---
    ...
    # Insert CITIES
    cursor.executemany("""
        INSERT INTO CITIES (id, name, state, lat, long)
        VALUES (:id, :name, :state, :lat, :long)
    """, cities_data)
    ...
    cursor.executemany("""
        INSERT INTO OPENING_HOURS (opens_at, closes_at, place_id, day_of_week)
        VALUES (:opens_at, :closes_at, :place_id, :day_of_week)
    """, opening_hours_rows)

    # Insert MIN_INTEREST
    cursor.executemany("""
        INSERT INTO MIN_INTEREST (place_id, architecture, history, spiritual, nature, culture)
        VALUES (:place_id, :architecture, :history, :spiritual, :nature, :culture)
    """, min_interest_rows)

    # Insert FESTIVALS
    cursor.executemany("""
        INSERT INTO FESTIVALS (id, name, start_date, end_date, city_id, description)
        VALUES (:id, :name, :start_date, :end_date, :city_id, :description)
    """, festivals_data)

    # Insert USERS_INPUT
    cursor.executemany("""
        INSERT INTO USERS_INPUT (id, gps_location, start_date, start_time, end_time, age)
        VALUES (:id, :gps_location, :start_date, :start_time, :end_time, :age)
    """, users_input_data)
    ...
    # --- 8. Export CSV Files for Each Table ---
    ...
    # 2. places.csv (strict match to user request + relational enrichments)
